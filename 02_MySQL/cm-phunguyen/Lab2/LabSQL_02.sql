...
HAVING COUNT(CASE WHEN C.MaHang = 'TP07' THEN 1 ELSE NULL END) =  1  
...
GROUP BY K.MaKhachHang
HAVING COUNT(CASE WHEN C.MaHang = 'TP07' THEN 1 ELSE NULL END) >= 1 
	AND COUNT(CASE WHEN C.MaHang = 'MM01' THEN 1 ELSE NULL END) =  0  
    AND COUNT(C.MaHang) >=2 ;
...
JOIN MATHANG M
	ON C.MaHang = M.MaHang
GROUP BY D.SoHoaDon
HAVING COUNT(CASE WHEN C.MaHang IN ('DT01','DT02','DT03','DT04') THEN 1 ELSE NULL END) = 4 
	AND COUNT(CASE WHEN C.MaHang IN ('DC01','TP03') THEN 1 ELSE NULL END) = 0 
