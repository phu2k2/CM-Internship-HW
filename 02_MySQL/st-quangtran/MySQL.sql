...
ALTER TABLE KHACHHANG MODIFY COLUMN DIenThoai VARCHAR(11);

-- ALTER ADD CONSTRANT
ALTER TABLE KHACHHANG ADD CONSTRAINT CHECK_DienThoai_Start_0 CHECK (DIenThoai REGEXP '^0');

-- ALTER DELETE CONSTRANT
ALTER TABLE KHACHHANG DROP CONSTRAINT CHECK_DienThoai_Start_0;

-- SELECT LIST TenCongTy OF NHACUNGCAP
SELECT TenCongTy From NHACUNGCAP;


-- SELECT MaHang, TenHang, SoLuong > 10 OF MATHANG
SELECT MaHang, TenHang, SoLuong
FROM MATHANG 
WHERE SoLuong > 10 AND DonViTinh = 'Cái';

-- SELECT HoTen, DiaChi, YEAR START WORK OF NHANVIEN
SELECT CONCAT(Ho, ' ', Ten) AS HoTen, DiaChi, YEAR(NgayLamViec) 
FROM NHANVIEN;

-- SELECT MaHang,TenHang OF MATHANG WHERE GiaHang > 100000 AND SoLuong < 50
